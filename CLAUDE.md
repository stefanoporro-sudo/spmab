...
**Raffreddamento per famiglia tematica** (`Topic.family` in `lib/social-topics.ts`, aggiunto 05/08/2026 dopo la ripetizione sul sale): oltre al confronto per testo esatto, `pickNextTopic()` ora raggruppa gli argomenti che condividono lo stesso soggetto specifico sotto un'etichetta `family` (es. `"sale"` per i 3 topic sul sale, `"semola-rimacinata"` per i 2 sulla semola) — se `family` non è impostato ma `ingredient` sì, la famiglia è implicitamente il valore di `ingredient` (due topic sullo stesso ingrediente di pregio in angoli diversi, es. storia + miti sulla stessa 'Nduja di Spilinga, sono automaticamente la stessa famiglia). Finché ci sono ancora argomenti mai usati, la rotazione preferisce quelli la cui famiglia non è stata toccata negli ultimi 21 giorni; se il raffreddamento svuoterebbe il pool disponibile, ripiega comunque su un argomento mai usato (mai su un blocco totale della generazione). Se in futuro emerge un'altra famiglia nascosta (stesso schema: soggetto specifico ripreso da angoli diversi), va semplicemente aggiunto `family` ai topic coinvolti — non serve toccare la logica di `pickNextTopic()`.
