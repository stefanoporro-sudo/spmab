...
⚠️ **Anche i 300 argomenti possono nascondere ripetizioni tematiche sotto voci diverse** (verificato più volte: "idratazione" duplicata sotto 6 voci, "grissini" sotto 2 — generico #65 e "rubatà torinesi" #83 — "pietra refrattaria" sotto 2 — #26 tecnica e #106 attrezzatura — e "sale" sotto 3, uscito due volte a 4 giorni di distanza il 3-5/08/2026 nonostante 226 argomenti su 300 non fossero ancora mai stati usati: tutte tecnicamente distinte come testo quindi mai bloccate dalla rotazione, ma percepite come la stessa cosa). Se un tema torna a sembrare ripetitivo, il primo sospetto è una duplicazione nascosta in `TOPICS`, non un bug della rotazione — e **non è un problema di quantità**: a inizio agosto 2026 solo 74 argomenti su 300 risultavano mai usati, quindi raddoppiare la lista a 600 non avrebbe risolto nulla (il problema non è esaurire gli argomenti, è che due argomenti diversi possono condividere lo stesso soggetto). Per trovare doppioni di testo sistematicamente (non aspettare che saltino fuori uno alla volta): estrarre le parole significative di ogni topic (>5 lettere, escludendo parole generiche di settore come pizzeria/farina/storia/tecnica) e cercare quelle condivise da esattamente 2 argomenti — segnala i doppioni veri senza il rumore delle parole di categoria naturalmente ricorrenti in una lista di 300 voci sulla pizza. Attenzione: questo metodo non cattura parole corte come "sale" o "olio" (sotto le 5 lettere) — vanno controllate a mano.

**Raffreddamento per famiglia tematica** (`Topic.family` in `lib/social-topics.ts`, aggiunto 05/08/2026 dopo la ripetizione sul sale, esteso 11/08/2026 dopo la ripetizione sulla focaccia genovese): oltre al confronto per testo esatto, `pickNextTopic()` ora raggruppa gli argomenti che condividono lo stesso soggetto specifico sotto un'etichetta `family` — se `family` non è impostato ma `ingredient` sì, la famiglia è implicitamente il valore di `ingredient` (due topic sullo stesso ingrediente di pregio in angoli diversi, es. storia + miti sulla stessa 'Nduja di Spilinga, sono automaticamente la stessa famiglia). Finché ci sono ancora argomenti mai usati, la rotazione preferisce quelli la cui famiglia non è stata toccata negli ultimi 21 giorni; se il raffreddamento svuoterebbe il pool disponibile, ripiega comunque su un argomento mai usato (mai su un blocco totale della generazione).

Famiglie attualmente taggate a mano (oltre alle ~12 derivate automaticamente da `ingredient` condiviso): `sale` (#30, #40, #59), `semola-rimacinata` (#42, #72), `focaccia-genovese` (#66, #84, #149 — lo stesso soggetto ripreso in tecnica/panificazione/storia, esattamente lo schema che ha causato la ripetizione dell'11/08/2026), `autolisi` (#1, #64), `food-cost` (#111, #112), `gabriele-bonci` (#144, #145, #188), `pizza-margherita` (#152, #269), `pizza-sottile-leggerezza` (#241, #267 — miti e faq che rispondono alla stessa identica domanda), `grani-antichi` (#162, #247).

⚠️ **Il tagging delle famiglie non taggate da `ingredient` è manuale e quindi incompleto per costruzione**: ogni volta che emerge una nuova ripetizione percepita, il sospetto è sempre lo stesso — un'altra famiglia nascosta in `TOPICS` mai taggata — non un bug della logica di raffreddamento (verificata via simulazione ogni volta e sempre corretta sui casi noti). Il modo giusto di verificare non è rileggere il codice ma **controllare i dati reali**: cercare nei 300 argomenti altri topic che condividono la stessa parola chiave specifica (nome di piatto, preparazione, persona citata, tecnica con nome proprio — non categorie generiche come farina/forno/impasto, che coprono legittimamente decine di argomenti distinti), poi aggiungere `family` ai topic coinvolti. Non serve mai toccare la logica di `pickNextTopic()` per farlo.
