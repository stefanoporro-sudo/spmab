...
Ometti il campo "suggested_recipe" solo se l'ingrediente non si presta davvero a una ricetta pizza/pane/focaccia.\n`
    ...
    return `Scrivi una caption per un post Instagram/Facebook come contenuto originale (non parte da un articolo specifico del sito).
...
async function generateDraft(slot: string) {
  ...
  try {
    ...
    // Per gli standalone l'argomento è già garantito diverso dalla rotazione forzata sui 300
    // argomenti: qui basta evitare aperture di caption identiche. Per post/ricetta l'angolo e
    // la tesi restano una scelta libera di Claude, quindi servono i controlli più stringenti.
    const collides =
...
async function finalizeDraft(source: SourceContent, parsed: ParsedCaption, slot: string) {
  ...
  // Per gli standalone il sotto-argomento salvato è sempre quello assegnato dalla rotazione
  // (non quello che Claude potrebbe riformulare), così il tracciamento anti-ripetizione sui
  // 300 argomenti resta un confronto testuale esatto e affidabile.
