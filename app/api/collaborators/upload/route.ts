...
export async function POST(req: NextRequest) {
  ...
  const fileName = `foto-${Date.now()}-${safeName}.${ext}`;
  ...
  if (error) return NextResponse.json({ error: "Errore nel caricamento: " + error.message }, { status: 500 });
